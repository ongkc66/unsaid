...
export default function PulsePage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'questions', filter: `team_id=eq.${team.id}` }, () => load())
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'answers' }, () => load())
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'teams', filter: `id=eq.${team.id}` }, () => load())
      .subscribe()
  ...
  const themeFrequency = (() => {
    ...
    insights.forEach((entry) => entry.themes.forEach((t) => counts.set(t, (counts.get(t) ?? 0) + 1)))
  ...
  const portraitSection = (
    <section>
      <div className="flex items-center justify-between mb-3">
        <p className="text-xs text-cream-muted uppercase tracking-widest">Team portrait</p>
        {hasSummary && team.summary_source_count > 0 && (
          <p className="text-xs text-cream-muted">
            {team.summary_source_count} insight{team.summary_source_count === 1 ? '' : 's'}
            {team.summary_generated_at ? ` · ${formatRelative(team.summary_generated_at)}` : ''}
          </p>
        )}
      </div>
      {hasSummary ? (
        <div className="relative rounded-2xl p-px" style={{ background: 'linear-gradient(135deg, #6366F1 0%, #818cf8 50%, #4338ca 100%)' }}>
            ...
            {team.summary_themes.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {team.summary_themes.map((t) => (
                  <span key={t} className="text-sm text-accent bg-accent/10 border border-accent/25 rounded-full px-3 py-1">{t}</span>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="bg-surface border border-white/5 rounded-2xl p-6 text-center">
          <p className="text-cream text-base font-medium mb-1.5">Your team's portrait appears once a few insights land.</p>
          <p className="text-cream-muted text-sm leading-relaxed">Ask a few questions and finish the threads. The picture sharpens with each insight.</p>
        </div>
      )}
    </section>
  )
  ...
  const heatmapSection = (
    <section className="flex flex-col gap-4">
      ...
      {totalEvents === 0 ? (
        <div className="bg-surface border border-white/5 rounded-2xl p-6 text-center">
          <p className="text-cream text-base font-medium mb-1.5">No activity yet.</p>
          <p className="text-cream-muted text-sm leading-relaxed">Ask a question or answer one to light up today.</p>
        </div>
      ) : (
        <>
  ...
  const ringSection = (
    <section className="bg-surface border border-white/5 rounded-2xl p-5">
      <p className="text-xs text-cream-muted uppercase tracking-widest mb-4">Completion</p>
  ...
  const originSection = (
    <section>
      <p className="text-xs text-cream-muted uppercase tracking-widest mb-3">Question origin</p>
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-surface border border-white/5 rounded-2xl p-5 flex flex-col gap-2">
          ...
          <p className="text-cream text-3xl font-semibold tabular-nums leading-none">{totalQuestions - aiQuestions}</p>
        </div>
        <div className="bg-surface border border-white/5 rounded-2xl p-5 flex flex-col gap-2">
          <div className="flex items-center gap-1.5">
          ...
          <p className="text-accent text-3xl font-semibold tabular-nums leading-none">{aiQuestions}</p>
        </div>
      </div>
    </section>
  )

  const recentInsightsSection = recentInsights.length > 0 ? (
    <section>
      <p className="text-xs text-cream-muted uppercase tracking-widest mb-3">Recent insights</p>
      <div className="flex flex-col divide-y divide-white/5 bg-surface border border-white/5 rounded-2xl overflow-hidden">
        {recentInsights.map((entry) => (
          <Link
            key={entry.questionId}
            href={`/team/${code}/question/${entry.questionId}`}
            className="flex items-start gap-3 px-4 py-3 hover:bg-white/2 transition-colors group"
          >
            <span className="w-1.5 h-1.5 rounded-full bg-accent flex-shrink-0 mt-1.5" />
            <p className="text-cream-muted text-sm leading-snug flex-1 line-clamp-2 group-hover:text-cream transition-colors">
              {entry.insightText}
            </p>
          </Link>
        ))}
  ...
  return (
    <main className="min-h-screen flex flex-col max-w-2xl lg:max-w-5xl mx-auto pb-28">
      ...
      {/* Page header — both layouts */}
      <div className="px-4 mb-6">
        <p className="text-xs text-cream-muted uppercase tracking-widest mb-1">Pulse</p>
        <h2 className="text-cream text-xl font-medium leading-snug">Your team's rhythm and character.</h2>
      </div>

      {/* ── Desktop: two-column dashboard ── */}
      <div className="hidden lg:grid lg:grid-cols-[3fr_2fr] lg:gap-8 px-4">
        {/* Left col: portrait → heatmap → suggestion tracker */}
        ...
        {/* Right col: ring → origin stats → themes → recent insights */}
      ...
      <div className="lg:hidden px-4 flex flex-col gap-8">
        {portraitSection}
        <div className="h-px bg-white/5" />
        <ThemeFrequency themes={themeFrequency} totalInsights={insights.length} />
        <div className="h-px bg-white/5" />
